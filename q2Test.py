...
def addUserScore(id, score):
    if not pd.isna(id):
        if id not in userVecs.keys():
            userVecs[id] = []
        userVecs[id].append(score)

stdDict = newRankDict()
for index, row in df.iterrows():
    if row['奖项'] == '一等奖':
        stdDict[int(row['名次'])] = row['专家一标准分（二）'] + row['专家二标准分（二）'] + row['专家三标准分（二）']
    addUserScore(row['专家一编码'], row['专家一原始分'])
    addUserScore(row['专家二编码'], row['专家二原始分'])
    addUserScore(row['专家三编码'], row['专家三原始分'])
    addUserScore(row['专家四编码'], row['专家四原始分'])
    addUserScore(row['专家五编码'], row['专家五原始分'])
    addUserScore(row['专家一编码（二）'], row['专家一原始分（二）'])
    addUserScore(row['专家二编码（二）'], row['专家二原始分（二）'])
    addUserScore(row['专家三编码（二）'], row['专家三原始分（二）'])

print(calcRankDiff(stdDict))
