...
### 基于二阶段标准分极差和复议后极差找规律
题里说了，第二阶段通过复议调整了极差，所以复议前后极差的变化是可以参考的。但我做了几个模型，结果如下（下面展示基于`数据2.2`，因为2.2数据量比较多。2.1的结论是一样的）：
...
Residuals:
    Min      1Q  Median      3Q     Max 
-11.107  -4.202   0.718   4.309   8.504 

Coefficients:
                         Estimate Std. Error t value Pr(>|t|)    
(Intercept)             11.342834   1.265857   8.961   <2e-16 ***
df$第二次评审标准分极差  0.001018   0.047810   0.021    0.983 
```
一点关系没有
...
Residuals:
     Min       1Q   Median       3Q      Max 
-11.3240  -3.9690   0.8268   4.1089   8.3996 

Coefficients:
              Estimate Std. Error t value Pr(>|t|)    
(Intercept)   16.21068    4.86037   3.335 0.000946 ***
df$二阶段成绩 -0.02624    0.02630  -0.998 0.319062    
```
没关系
...
二者都是左偏+右拖尾（虽然这个结论没啥用）。然后题里要求调整第一阶段不高不低分段的极差，我们先看下“不高不低分段”到底是多少，取百分位区间15%~85%：
* 数据2.1百分位区间：
```
